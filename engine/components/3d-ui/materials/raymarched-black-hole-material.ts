...
// Adapted from the provided black hole shader with raymarching and gravitational lensing
const vertexShader = `
varying vec2 vUv;
...
vec4 background(vec3 ray)
{
    ...
    // Simple nebula effect (no texture channel available)
    vec2 nebCoord = uv * 1.5;
    float nebNoise = value(nebCoord, 10.0);
    vec4 nebulae = vec4(nebNoise * 0.3, nebNoise * 0.2, nebNoise * 0.4, nebNoise);
...
void main()
{
    ...
    for( int i=0; i<AA; i++ )
    {
        ...
        vec3 pos = vec3(0.,0.05,-(20.*0.5-10.)*(20.*0.5-10.)*.05); 
        vec2 angle = vec2(time*0.1,.2);      
        angle.y = (2.*0.5)*3.14 + 0.1 + 3.14;
        ...
        for(int disks = 0; disks< 20; disks++) //steps
        {
            for (int h = 0; h < 6; h++) //reduces tests for exit conditions (to minimise branching)
            {
                ...
                float bendForce = stepDist * invDistSqr * _Size * 0.625 * distortion;  //bending force
    ...
    colOut.rgb *= intensity;
    
    gl_FragColor = colOut;
}
`

export class RaymarchedBlackHoleMaterial extends THREE.ShaderMaterial {
  constructor() {
    super({
      uniforms: {
        time: { value: 0 },
...
extend({ RaymarchedBlackHoleMaterial })

export default RaymarchedBlackHoleMaterial 